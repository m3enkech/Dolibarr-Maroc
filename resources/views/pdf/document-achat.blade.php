...
<body>
@php
    $titles = ['commande' => 'Bon de commande', 'reception' => 'Bon de réception', 'facture' => 'Facture fournisseur'];
...
<table class="header">
    <tr>
        ...
        <td style="text-align: right;">
            <div class="doc-title">{{ $titles[$document->type] ?? 'Document' }}</div>
            ...
            @endif
            @if ($document->ref_fournisseur)
                <div class="muted">Réf. fournisseur : {{ $document->ref_fournisseur }}</div>
            @endif
        </td>
    </tr>
</table>
...
<table style="width: 100%;">
    <tr>
        <td style="width: 48%; vertical-align: top;">
            @if ($document->entrepot)
                <div class="box">
                    <div class="box-title">Livraison</div>
                    <div style="font-weight: bold;">{{ $document->entrepot->name }}</div>
                    @if ($document->entrepot->address)<div class="muted">{{ $document->entrepot->address }}</div>@endif
                </div>
            @endif
        </td>
        <td style="width: 4%;"></td>
