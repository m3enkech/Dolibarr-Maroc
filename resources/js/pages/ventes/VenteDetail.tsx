...
export default function VenteDetail() {
    ...
    const invalidate = () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['vente-detail', id] });
    };

    const onError = (err: any) => {
        const messages = err?.response?.data?.errors;
        setError(
            messages ? (Object.values(messages).flat() as string[]).join(' ') : 'Action impossible.',
        );
    };

    const action = useMutation({
        mutationFn: ({ path, body }: { path: string; body?: Record<string, unknown> }) =>
            api.post(`/ventes/documents/${id}/${path}`, body ?? {}),
        onSuccess: () => {
            setError(null);
            invalidate();
        },
    ...
    const supprimer = useMutation({
        mutationFn: () => api.delete(`/ventes/documents/${id}`),
        onSuccess: () => {
            invalidate();
            navigate('/ventes');
        },
    ...
    const telechargerPdf = async () => {
        const response = await api.get(`/ventes/documents/${id}/pdf`, { responseType: 'blob' });
    ...
    return (
        <div className="max-w-5xl space-y-4">
            <div>
                <Link to={`/ventes?type=${doc.type}`} className="text-sm text-emerald-600 hover:underline">
                    ← Retour aux {TYPE_LABELS[doc.type].toLowerCase()}s
                </Link>
                <div className="mt-2 flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`rounded px-2 py-0.5 text-xs ${statutClasses(doc.statut)}`}>
                                {statutLabel(doc)}
                            </span>
                        </div>
                        <p className="mt-1 text-sm text-slate-500">
                            {doc.tiers?.name} — {doc.date_document}
                            {doc.date_echeance && ` · Échéance : ${doc.date_echeance}`}
                            {doc.source && (
                                <>
                                    {' '}· Issu de{' '}
                                    <Link to={`/ventes/${doc.source.id}`} className="text-emerald-600 hover:underline">
                                        {doc.source.code}
                                    </Link>
                                </>
                            )}
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                        {doc.statut === 'brouillon' && (
                            <>
                                ...
                                <Link to={`/ventes/${doc.id}/modifier`} className={btnSecondary}>
                                    Modifier
                                </Link>
                                <button
                        ...
                        {doc.type === 'devis' && doc.statut === 'valide' && (
                            <>
                                <button
                                    onClick={() => action.mutate({ path: 'statut', body: { statut: 'accepte' } })}
                                    className={btnPrimary}
                                >
                                    Accepter
                                </button>
                                <button
                                    onClick={() => action.mutate({ path: 'statut', body: { statut: 'refuse' } })}
                                    className={btnDanger}
                                >
                                    Refuser
                                </button>
                        ...
                        {doc.type === 'devis' && ['valide', 'accepte'].includes(doc.statut) && (
                            <>
