    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <p className="mt-0.5 text-sm text-slate-500">Passée le {data.date}</p>
            </div>

            {data.etat === 'en_attente_confirmation' && (
                <div className="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Votre commande a bien été transmise. Le grossiste va la confirmer, et les montants
                    définitifs vous seront communiqués à ce moment-là.
                </div>
            )}

            {enCoursDeLivraison && (
                <div className="rounded-lg bg-indigo-50 px-4 py-3 text-sm text-indigo-800">
                    Une partie de votre commande a été livrée. Le reste vous sera livré prochainement.
                </div>
            )}
                ...
                <table className="w-full text-left text-sm">
                    <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                        <tr>
                            ...
                            <th className="px-4 py-2.5 text-right">Commandé</th>
                            <th className="px-4 py-2.5 text-right">Livré</th>
                            ...
                            <tr key={i}>
                                <td className="px-4 py-3">
                                    ...
                                    <span className="block text-xs text-slate-400">
                                        {formatMAD(l.prix_unitaire)} l'unité
                    ...
                    <tfoot className="border-t-2 border-slate-200 bg-slate-50">
                        <tr>
                            <td colSpan={3} className="px-4 py-3 text-right font-medium text-slate-700">Total TTC</td>
            ...
            {data.notes && (
                <div className="rounded-xl bg-white p-4 shadow-sm">
                    <h2 className="text-sm font-medium text-slate-900">Votre note</h2>
