...
export default function Abonnement() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-xl font-semibold text-slate-900">{t('Mon abonnement')}</h1>
                <p className="mt-1 text-sm text-slate-500">{t('Votre formule, son statut et vos factures d’abonnement.')}</p>
            </div>

            <section className="rounded-xl bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <div className="text-xs uppercase tracking-wide text-slate-400">{t('Formule')}</div>
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${st.cls}`}>{t(st.label)}</span>
                </div>
                <div className="mt-4 grid gap-4 border-t border-slate-100 pt-4 text-sm sm:grid-cols-2">
                    <div>
                        <div className="text-xs text-slate-400">{t('Prochaine échéance')}</div>
                        <div className="mt-0.5 text-slate-700">{fr(s.current_period_end)}</div>
                    </div>
                    {s.trial_ends_at && (
                        <div>
                            <div className="text-xs text-slate-400">{t('Fin de l’essai')}</div>
            ...
            <section className="rounded-xl bg-white shadow-sm">
                <div className="border-b border-slate-100 px-5 py-3 text-sm font-semibold text-slate-700">
                    {t('Factures d’abonnement')}
                </div>
                {!data.factures.length ? (
                    <div className="px-5 py-8 text-center text-sm text-slate-400">{t('Aucun paiement enregistré pour le moment.')}</div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-slate-100 text-left text-xs uppercase text-slate-400">
                                    ...
                                    <th className="px-5 py-3">{t('Mode')}</th>
                                    <th className="px-5 py-3">{t('Référence')}</th>
                                    <th className="px-5 py-3 text-right">{t('Montant')}</th>
                                    <th className="px-5 py-3 text-right">{t('Facture')}</th>
                                </tr>
                            </thead>
